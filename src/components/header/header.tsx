  ...
  return (
    <header
      className={`fixed transition-all duration-300 ease-in-out ${
        expandedHeader
          ? "top-0 mx-2 my-4 lg:mx-26 2xl:mx-[15vw]"
          : "top-4 mx-6 lg:mx-32 2xl:mx-[16vw]"
      } ${className}`}
      ...
      <div
        className={`flex flex-col items-center justify-center rounded-[2.2rem] select-none border-[1px] ${
          ...
          expandedHeader ? "border-transparent bg-main-bg" : "border-header-border-color bg-header-bg"
        }`}>
        <div
        ...
        <Menu
          ...
          isOpen={menuOpen}
          className={`w-full ${expandedHeader ? "h-screen" : ""}`}
        />
      </div>
    </header>
  );
