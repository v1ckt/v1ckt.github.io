...
export default function StackSection({ className }: StackSectionProps) {
  return (
    <section
      className={`flex flex-col items-end justify-center px-6 md:px-24 text-center w-full gap-8
        ...
        <div className="flex flex-col items-center justify-center gap-12 w-full">
          ...
          <ul className="flex flex-row gap-6 md:gap-0 w-full justify-center md:justify-between items-center flex-wrap">
            {namedStack.map((ic, index) => (
              <li key={index}>
                <SquaredIcon icon={ic.data.src} size={7} color={ic.color} />
              </li>
            ))}
          </ul>
        </div>
      </div>
      <Clink href="#" title="All my skills" />
    </section>
  );
}
